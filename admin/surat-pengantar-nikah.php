...
// $surat = query("SELECT s.*, ms.nama_surat FROM surat s LEFT JOIN ms_surat ms ON ms.id = s.id_jenis WHERE s.id_jenis = 5");
...
END) AS status
FROM surat s LEFT JOIN ms_surat ms ON ms.id = s.id_jenis WHERE s.id_jenis = 5 AND s.validatedAt IS NOT NULL");
} else {
...
(CASE
 ...
 WHEN s.validatedAt IS NOT NULL AND s.approvedAt IS NOT NULL THEN 'Selesai'
  WHEN s.rejectedAt IS NOT NULL THEN 'Ditolak'
END) AS status
FROM surat s LEFT JOIN ms_surat ms ON ms.id = s.id_jenis WHERE s.id_jenis = 5");
}
?>
